...
retry_s3_operation(Fun, MaxRetries, _BaseDelay, Attempt) when Attempt >= MaxRetries ->
    ...
    case Fun() of
        {error, Reason} = Error ->
            ?ERROR("[s3_api] S3 operation failed after ~p retries: ~p", [MaxRetries, Reason]),
            Error;
        Result ->
            Result
    end;
        ...
        {error, Reason} = Error ->
            case is_retryable_error(Reason) of
                true ->
                    ...
                    ?WARNING("[s3_api] S3 operation failed (attempt ~p/~p): ~p, retrying after ~p ms",
                                  [Attempt + 1, MaxRetries, Reason, Delay]),
                    timer:sleep(Delay),
                    retry_s3_operation(Fun, MaxRetries, BaseDelay, Attempt + 1);
                false ->
                    ?ERROR("[s3_api] Non-retryable S3 error: ~p", [Reason]),
                    Error
            end;
        Result ->
            Result
...
put_object(BucketId, Prefix, ObjectKey, BinaryData, Options)
	...
	{error, Reason} ->
	    ?ERROR("[s3_api] Failed to put object ~p/~p/~p: ~p",
