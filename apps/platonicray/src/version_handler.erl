...
response(Req0, <<"HEAD">>, BucketId, User, T0) ->
	...
	{error, Reason} ->
	    ?ERROR("[version_handler] head_object failed ~p/~p: ~p", [BucketId, ?DB_VERSION_KEY, Reason]),
...
response(Req0, <<"GET">>, BucketId, _User, _T0) ->
    case s3_api:head_object(BucketId, ?DB_VERSION_KEY) of
	{error, Reason} ->
	    ?ERROR("[version_handler] get_object failed ~p/~p: ~p", [BucketId, ?DB_VERSION_KEY, Reason]),
...
response(Req0, <<"POST">>, BucketId, Prefix, ObjectKey, User, Meta, Version, T0) ->
	...
	ok ->
		...
		lock ->
		    ?WARNING("[version_handler] Can't update index: lock exists"),
	...
	{error, Reason} ->
	    ?ERROR("[version_handler] Can't put object ~p/~p/~p: ~p",
			[BucketId, Prefix, ObjectKey, Reason]),
	    ?WARNING("[version_handler] Can't update index: ~p", [Reason]),
	    js_handler:too_many(Req0)
    end.
...
validate_object_key(BucketId, Prefix, ObjectKey, Version)
	...
	{error, Reason} ->
	    ?ERROR("[version_handler] head_object failed ~p/~p: ~p",
