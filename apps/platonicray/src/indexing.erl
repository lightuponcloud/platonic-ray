...
get_index(BucketId, Prefix0)
    ...
    case s3_api:get_object(BucketId, PrefixedIndexFilename) of
        {error, Reason} ->
	    ?ERROR("[indexing] get_object error ~p/~p: ~p",
...
get_object_record(IndexContent, ObjectKey) when erlang:is_binary(ObjectKey) ->
	...
	[Rec] -> Rec;
	[_ | _] = Recs ->
	    ?WARNING("[indexing] Multiple records for key ~p: ~p", [ObjectKey, Recs]),
	    lists:nth(1, Recs)
    end.
...
update(BucketId, Prefix0, Options, RiakOptions)
	...
	{error, Reason0} ->
	    ?ERROR("[indexing] head_object failed ~p/~p: ~p",
	...
	not_found ->
	    ...
	    case Response of
		{error, Reason1} -> ?ERROR("[indexing] Can't put object ~p/~p/~p: ~p",
	    ...
	    case Response of
		{error, Reason2} -> ?ERROR("[indexing] Can't put object ~p/~p/~p: ~p",
...
remove_previous_version(BucketId, GUID, UploadId0, Version) when erlang:is_list(Version) ->
	...
	case dvvset:values(Version) of
	    [VT] -> VT;
	    _ ->
		?ERROR("[indexing] Invalid version vector: ~p", [Version]),
		undefined
	end,
	...
	fun(PreviousOne) ->
		...
		{UploadId2, _Date, _VV} ->
			...
			ok ->
			    ...
			    case Response of
				{error, Reason} -> ?ERROR("[indexing] Can't put object ~p/~p/~p: ~p",
...
add_dvv(BucketId, GUID, UploadId, Version, UserId, UserName)
    ...
    case remove_expired_dvv_lock(BucketId, GUID) of
	lock ->
	    ?WARNING("[indexing] Can't remove expired dvv lock: ~p/~p", [BucketId, GUID]),
	    lock;
	ok ->
		...
		false ->
		    ...
		    case Response of
			{error, Reason} ->
			    ?ERROR("[indexing] Can't put object ~p/~p/~p: ~p",
					[BucketId, RealPrefix, ?DVV_INDEX_FILENAME, Reason]),
			    {error, Reason};
			_ -> {UploadId, List0}
		    end
