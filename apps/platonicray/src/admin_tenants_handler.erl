...
get_tenant(TenantId0) when erlang:is_list(TenantId0) ->
	...
	_ ->
		...
		{error, Reason} ->
		    ?ERROR("[admin_tenants_handler] get_object error ~p/~p: ~p",
...
new_tenant(Req0, Tenant0) ->
	...
	{error, Reason} ->
	    ?ERROR("[download_handler] head_object ~p/~p: ~p",
			[?SECURITY_BUCKET_NAME, PrefixedTenantId, Reason]),
	...
	not_found ->
	    ...
	    case Response of
		{error, Reason} -> ?ERROR("[admin_tenants_handler] Can't put object ~p/~p/~p: ~p",
...
edit_tenant(Req0, Tenant) ->
	...
	{error, Reason0} ->
	    ?ERROR("[admin_tenants_handler] head_object failed ~p: ~p", [PrefixedLockFilename, Reason0]),
	...
	not_found ->
	    ...
	    case Response of
		{error, Reason1} -> ?ERROR("[admin_tenants_handler] Can't put object ~p/~p/~p: ~p",
	    ...
	    case Response of
		{error, Reason} ->
		    ?ERROR("[admin_tenants_handler] Can't put object ~p/~p/~p: ~p",
