...
check_src_dir(BucketId, Prefix) ->
	...
	{error, Reason} ->
	    ?ERROR("[rename_handler] get_object error ~p/~p: ~p",
...
copy_delete(BucketId, PrefixedSrcDirectoryName, PrefixedDstDirectoryName, PrefixedObjectKey0) ->
	...
	_ ->
		...
		false ->
			...
			{error, Reason} -> ?ERROR("[rename_handler] Can't delete object ~p/~p: ~p", [BucketId, PrefixedObjectKey0, Reason]);
			{ok, _} -> ok
		    end,
		    PrefixedObjectKey2,
		    ok
...
rename_pseudo_directory(BucketId, Prefix0, PrefixedSrcDirectoryName, DstDirectoryName0,
	...
	fun(PrefixedObjectKey) ->
		...
		true ->
			...
			_ ->
				...
				{error, Reason} ->
				    ?ERROR("[rename_handler] Can't delete object ~p/~p: ~p", [BucketId, PrefixedObjectKey, Reason]),
				    ok;
				{ok, _} -> ok
			    end
		    end
	...
	true ->
		...
		true ->
			...
			lock ->
			    ?WARNING("[rename_handler] Can't move directory, as lock exists: ~p/~p", [BucketId, Prefix0]),
			    lock;
...
delete_source_object(BucketId, SrcPrefix, SrcObjectKey, DstObjectKey) ->
	...
	case s3_api:delete_object(BucketId, PrefixedSrcObjectKey) of
	    {error, Reason0} ->
		?ERROR("[rename_handler] Can't delete object ~p/~p: ~p",
	...
	_ ->
		...
		{error, Reason} ->
		    ?ERROR("[rename_handler] Can't delete lock object ~p/~p: ~p",
...
rename_object(BucketId, Prefix0, SrcObjectKey0, DstObjectName0, User, IndexContent) ->
	...
	undefined ->
		...
		{false, Metadata0} ->
			...
			{error, Reason3} ->
			    ?ERROR("[rename_handler] Can't put object ~p/~p/~p: ~p",
			...
			ok ->
				...
				lock ->
				    ?WARNING("[rename_handler] Can't update index, as lock exists: ~p/~p",
						  [BucketId, Prefix0]),
				    lock;
				_ ->
