		...
		fun(J) ->
			...
			true ->
				...
				{SQL0, SQL1} ->
				    case sqlite3:sql_exec(DbName, SQL0) of
					{error, _, Reason0} ->
					    ?ERROR("[sqlite_server] SQL: ~p Error: ~p", [SQL0, Reason0]),
					    {Module, Func, Args};  %% adding it back to queue
					_ ->
					    case sqlite3:sql_exec(DbName, SQL1) of
						{error, Reason1} ->
						    ?ERROR("[sqlite_server] SQL: ~p Error: ~p", [SQL1, Reason1]),
						    {Module, Func, Args};  %% adding it back to queue
						_ -> []
					    end
				    end;
				SQL ->
					...
					{error, _, Reason2} ->
					    ?ERROR("[sqlite_server] SQL: ~p Error: ", [SQL, Reason2]),
					    {Module, Func, Args};  %% adding it back to queue
					_Result -> []
				    end
			    end;
...
open_db(BucketId, DbName) ->
    TempFn = utils:get_temp_path(platonicray),
...
lock_db(BucketId, LockName) ->
	...
	IndexLockMeta ->
		...
		true ->
			...
			{error, Reason} ->
			    ?ERROR("Failed removing lock ~p/~p", [BucketId, LockName]),
			    {error, Reason};
			{ok, _} ->
