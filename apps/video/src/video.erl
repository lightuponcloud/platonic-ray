...
process_video(BucketId, ObjectKey) ->
    case download_file(BucketId, ObjectKey) of
        ...
        {RealPrefix, TempFn} ->
            ...
            TempOutput = filename:join(TempDir, ?WEBM_OBJECT_KEY),
            try
...
upload_webm_file(BucketId, RealPrefix, TempOutput) ->
    case file:read_file(TempOutput) of
        {ok, Data} ->
            OutputKey = filename:join(RealPrefix, ?WEBM_OBJECT_KEY),
            case s3_api:put_object(BucketId, RealPrefix, OutputKey, Data) of
                {error, Reason} ->
...
download_file(BucketId, ObjectKey) ->
	...
	{error, Reason} ->
	    ?ERROR("[video_transcoding] head_object failed ~p/~p: ~p", [BucketId, ObjectKey, Reason]),
	    {error, Reason};
	    ...
	    case List1 of
		 ...
		 [PrefixedObjectKey | NextKeys] ->
			...
			{ok, RequestId} ->
				...
				{http, Msg} ->
				    ?ERROR("[video_sup] error starting stream: ~p", [Msg]),
				    {error, stream_error}
			    end
		    end
	    end
...
receive_streamed_body(RequestId0, Pid0, BucketId, NextObjectKeys0, OutputFileName) ->
	...
	{http, {RequestId0, stream_end, _Headers0}} ->
		...
		[CurrentObjectKey|NextObjectKeys1] ->
			...
			not_found ->
			    ?ERROR("[video_transcoding] part not found: ~p/~p", [BucketId, CurrentObjectKey]),
			    {error, not_found};
			{ok, RequestId1} ->
			    receive
				...
				{http, Msg} ->
				    ?ERROR("[video_transcoding] stream error: ~p", [Msg]),
				    {error, stream_error}
			    end
		    end
	    end;
	{http, Msg} ->
	    ?ERROR("[video_sup] cant receive stream body: ~p", [Msg]),
	    {error, stream_error}
    end.
